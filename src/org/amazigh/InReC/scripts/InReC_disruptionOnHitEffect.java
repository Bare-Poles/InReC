			...
			if (target instanceof ShipAPI) {
				if (shieldHit) {
					...
					if (!((ShipAPI) target).isAlive() || ((ShipAPI) target).isPiece()) {
                    	for (int i=0; i < 4; i++) {
                        	// if the target is dead or a piece, just deal all the arc damage at the impact point - KoL modules >:(
            				engine.applyDamage(target, point, projectile.getDamageAmount() * 0.2f, DamageType.ENERGY, 0, true, false, projectile.getSource());
                    	}
                    	Vector2f nebVel2 = MathUtils.getRandomPointInCircle(target.getVelocity(), MathUtils.getRandomNumberInRange(2f, 10f));
                		engine.addSwirlyNebulaParticle(point,
                				nebVel2,
                				10f,
                				1.5f,
