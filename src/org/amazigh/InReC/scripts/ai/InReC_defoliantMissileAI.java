...
public class InReC_defoliantMissileAI implements MissileAIPlugin, GuidedMissileAI {
    ...
    */
    ...
    public InReC_defoliantMissileAI(MissileAPI missile, ShipAPI launchingShip) {
    ...
    public void advance(float amount) {
        ...
        if (ARMED) {
        	...
        	if (!FIRED) {
    			...
    			Global.getSoundPlayer().playSound("hurricane_mirv_split", 1.5f, 1.0f, loc, MISSILE.getVelocity());
    			// "hurricane_mirv_split", 1.4f, 1.2f

    			// impulse
    			MISSILE.getVelocity().set(MathUtils.getPointOnCircumference(null, 550f, MISSILE.getFacing()));
    			...
    			engine.addNebulaSmokeParticle(loc,
                		...
                		1.0f, //duration
                		new Color(173,167,155,100));
    			for (int i=0; i < 4; i++) {
                    Vector2f puffRandomVel = MathUtils.getRandomPointOnCircumference(null, MathUtils.getRandomNumberInRange(4f, 18f));
                	engine.addSmokeParticle(loc, puffRandomVel, MathUtils.getRandomNumberInRange(20f, 40f), 0.8f, 0.8f, new Color(111,105,89,100));
                    ...
                    for (int j=0; j < 2; j++) {
                    	...
                    	for (int jj=0; jj < 2; jj++) {
                        	...
                        	engine.addSmoothParticle(loc,
                        			...
                        			1.0f, //brightness
                        			1.2f, //duration
                        			new Color(255,210,100,255));
                    	}
                		// frontal smoke jet/trail
                    	for (int k=0; k < 4; k++) {
                    		float jetDist = (i * 18f) + (k * 4f) + MathUtils.getRandomNumberInRange(0f, 14f); // 22-36 - 88-102
                        	...
                        	engine.addNebulaSmokeParticle(loc,
                            		smokeRandomVelJ,
                            		MathUtils.getRandomNumberInRange(12f, 18f) - (i * 2), //size
                            		2.1f, //end mult
                            		0.5f, //ramp fraction
                            		0.45f, //full bright fraction
                            		0.2f + (jetDist * 0.02f), //duration
                            		new Color(173,167,155,jetAlpha));
                    	}
                    }
    			}
